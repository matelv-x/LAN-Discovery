...
if [ -z "$BACKUP_DIR" ] || [ ! -d "$BACKUP_DIR" ]; then
  BACKUP_DIR="$(find "$BACKUP_ROOT" -maxdepth 1 -type d -name 'sg1_v4_backup_lan_discovery_*' 2>/dev/null | sort | tail -n 1)"
fi

if [ -z "$BACKUP_DIR" ] || [ ! -d "$BACKUP_DIR" ]; then
  echo "ERROR: no backup found. Expected .last_backup_path or $BACKUP_ROOT/sg1_v4_backup_lan_discovery_*" >&2
  exit 1
fi
